...
| `UPLOADS_DIR` | Directory inside the container where uploaded login-page assets (logo, background) are stored. Auto-created on startup. Keep it on a mounted volume to persist (the bundled compose mounts `/data/uploads`). | `/data/uploads` | An absolute path inside the container. | |
...
| `LOG_FILE` | Path the combined log is written to. Each file is capped at 2 MB, then rotated; rotated files are kept for 7 days and then deleted. Put it on a mounted volume to keep logs across container recreations (the bundled compose mounts `/data/logs`). | `/data/logs/app.log` | An absolute path inside the container. | |
